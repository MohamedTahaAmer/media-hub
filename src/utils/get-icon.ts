...
let getTSXIcon = async (svgName: string, defaultIcon: string) => {
	...
	try {
		let { default: Icon } = (await import(`@/components/svgs/icons/${tsxIconName}`)) as {
			default: (_: LucideProps) => JSX.Element
		}
		return Icon
	} catch (error) {
		...
		let { default: Icon } = (await import(`@/components/svgs/icons/${defaultIcon}`)) as {
			default: (_: LucideProps) => JSX.Element
		}
		return Icon
	}
